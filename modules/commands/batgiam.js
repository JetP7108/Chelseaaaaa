...
async function makeImage({ one, two }) {
    ...
    batgiam_img.composite(circleOne.resize(100, 100), 375, 9).composite(circleTwo.resize(100, 100), 160, 92);
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("Vui lòng tag 1 người", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "Xin chúc mừng em đã vào biên chế nhà nước nha" + tag + 'Chúc em vui vẻ😆',
            mentions: [{
          tag: tag,
          id: mention
        }],
