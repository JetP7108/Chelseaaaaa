...
module.exports.run = async ({ api, event, Currencies }) => {
	...
	if (money >= 1000) {
		...
		var callback = function () {
					...
					}, event.threadID, () => fs.unlinkSync(__dirname + '/cache/sexy.jpg'), event.messageID);
				};
				request(res.data.data).pipe(fs.createWriteStream(__dirname + '/cache/sexy.jpg')).on("close", callback);
