module.exports.config = {
    ...
    description: "Ban thành viên vĩnh viễn khỏi nhóm (nhớ set qtv cho bot nha)\nAuthor: NTKhang",
...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
  ...
  var info = await api.getThreadInfo(threadID);
  if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return out(`Bạn không đủ quyền hạn để sử dụng lệnh này.`);
  var reason = "";
